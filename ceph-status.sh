...
case $1 in
  ...
  ;;
  rados_total)
    $rados_bin df | grep "total_space"| awk '{print $2}' |tr -d [:alpha:]
  ;;
  rados_used)
    $rados_bin df | grep "total_used"| awk '{print $2}' |tr -d [:alpha:]
  ;;
  rados_free)
    $rados_bin df | grep "total_avail"| awk '{print $2}' |tr -d [:alpha:]
  ;;
  mon)
    ceph_mon_get_active
  ;;
